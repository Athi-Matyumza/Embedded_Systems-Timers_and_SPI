...
/**
  ******************************************************************************
  * @file           : Timers_and_SPI.c
  * @brief          : Timers and SPI
  ******************************************************************************
  * @attention
  *
...
uint16_t addresses[] = {0x0000, 0x0001, 0x0002, 0x0003, 0x0004, 0x0005};

uint8_t current_address = 0; // index to current address in EEPROM memory & patterns array
...
int main(void)
{
  ...
  for (int index = 0; index < sizeof(patterns); index++)
  {
    write_to_address(addresses[index], patterns[index]);
  }

  /* USER CODE END 2 */

  int period_delays[] = {499, 999}; // STORING TWO PERIODS
  int temp;
  /* Infinite loop */
  ...
  while (1)
  {
    ...
    if ((LL_GPIO_ReadInputPort(Button0_GPIO_Port) & Button0_Pin) == 0)
    {
      ...
      htim16.Init.Period = period_delays[0]; // MODIFY PERIOD
      /* ALTERNATE DELAY IN ARRAY */
      temp = period_delays[0];
      period_delays[0] = period_delays[1];
      period_delays[1] = temp;

      HAL_TIM_Base_Init(&htim16); // UPDATE PERIOD IN THE TIMER
    }
  }
  /* USER CODE END 3 */
}
...
void TIM16_IRQHandler(void)
{
  ...
  uint8_t value = read_from_address(addresses[current_address]); // READ PATTERN VALUE FROM EEPROM MEMORY

  if (value == patterns[current_address]) // SAME WITH VALUE IN LOCAL ARRAY
  {                                       // Fail safe
    GPIOB->ODR = value;                   // DISPLAY
  }
  else
  {
  ...
  current_address = (current_address + 1) % sizeof(patterns); // MOVE WRAPPED INDEX IN MEMORY FORWARD
}
